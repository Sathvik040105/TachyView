...
class DataModule(Module):
    ...
    def loadFile(self, filename):
        ...
        if str(filename).lower().endswith('.vtk'):
            ...
            try:
                # Convert VTK to TXT
                num_points = vtk_to_txt(filename, txt_filename)
                print(f"Converted {num_points} points from {filename} to {txt_filename}")
                
                # Load the converted TXT file
                filename = txt_filename
            except Exception as e:
                print(f"Error converting VTK file: {e}")
                raise
        
        # Load the file (either original .txt or converted from .vtk)
        data = loadFile(str(filename))
        self.data.setData(data)
